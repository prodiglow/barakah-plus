...
interface ScholarDetailsPopProps {
  scholar: Scholar;
  onSelect?: (scholar: Scholar) => void;
}

const ScholarDetailsPop: React.FC<ScholarDetailsPopProps> = ({ scholar, onSelect }) => {
        ...
        <Grid size={{ xs: 12, md: 4 }}>
          <Paper variant="outlined" sx={{ p: 2, boxShadow: '0px 4px 6px rgba(0, 0, 0, 0.1)' }}>
            ...
            <Button
              fullWidth
              variant="contained"
              onClick={() => scholar && onSelect && onSelect(scholar)}
              sx={{
      ...
      <Box width="100%" mt={5}>
        ...
        <Box width="85%" mx="auto">
          ...
          >
            ...
            >
              {scholar.reviews.map((review) => review.comment).join(" ")}
            </Typography>
            <Typography variant="caption" color="text.secondary">
              Verified patient · 21 hours ago
            </Typography>
          </Box>
...
export default ScholarDetailsPop;
